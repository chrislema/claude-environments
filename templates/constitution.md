...
This repo uses the delivery-engine plugin:

- `/deliver vision.md spec.md` runs the judged pipeline; `/deliver-status` inspects it.
- `.delivery/` is the authoritative run state — never edit it by hand; use
  `scripts/stage.mjs`.
- Role boundaries come from the plugin's `policy/boundaries.json` and are enforced by
  hooks during runs.
- Stage outputs are judged against `rubrics/` — a failed gate bounces work with
